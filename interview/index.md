...
1. Client 发起一个 HTTPS 请求，连接 443 端口。这个过程可以理解成是请求公钥的过程。
...
因为每个 HTTP 请求都是独立的，与之前的请求或之后的请求没有直接关系。服务器不会自动保存每个请求的状态信息。这意味着服务器在处理每个请求时，不会记住之前的请求信息。

但在实际应用中，我们经常需要在多个请求之间保持状态。为了解决这个问题，通常使用以下几种方法：

- Cookies: 服务器通过在响应中设置 Cookie，将状态信息存储在客户端。客户端在后续请求中会自动发送这些 Cookie，从而实现状态管理。
- Session: 服务器在内存或数据库中保存会话信息，并通过 Cookie 或 URL 参数将会话 ID 传递给客户端。客户端在后续请求中发送会话 ID，服务器根据会话 ID 查找并恢复状态信息。
- Token: 使用 JWT（JSON Web Token）等令牌机制，在客户端和服务器之间传递状态信息。令牌通常包含加密的状态信息，客户端在后续请求中发送令牌，服务器解密并验证令牌以恢复状态信息。

### DNS 如何工作的

DNS 的作用就是通过域名查询到具体的 IP。DNS 协议提供的是一种主机名到 IP 地址的转换服务
