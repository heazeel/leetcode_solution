...
// if (!editor) {
//   vscode.window.showInformationMessage('no activate text editor');
//   return;
// }
// const selectedText = TextEditorDelegate.getSelectedText(editor);
// const cursorPrefixText = TextEditorDelegate.getCursorPrefixText(editor);
// const cursorSuffixText = TextEditorDelegate.getCursorSuffixText(editor);
// console.log({ selectedText, cursorPrefixText, cursorSuffixText });
// TextEditorDelegate.replaceSelectedText(editor, 'Hello World');

export class TextEditorDelegate {
  ...
  public static async insertTextBeforePreview(editor: TextEditor, content: string) {
    ...
    const previewFileUri = await FileHelper.writeTmpFile(filePath, currentContent);

    // 对比工作区和临时文件
    if (previewFileUri) {
      commands.executeCommand('vscode.diff', previewFileUri, currentDocument.uri, 'crow preview');
    }
  }
  ...
  // 移动光标至最后一行
  public static moveCursorToLastLine(document: TextDocument, editor: TextEditor) {
