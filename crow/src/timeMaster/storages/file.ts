...
import { DEFAULT_TIME_STORAGE_LIMIT } from '../utils/storage';
...
export async function sendLog() {
  ...
  const todayFile = getFilesFile(today);
  const todayFileExsit = await fse.pathExists(todayFile);
  if (!todayFileExsit) {
    let dayLimit = 1;
    while (dayLimit <= DEFAULT_TIME_STORAGE_LIMIT) {
      const day = moment().subtract(dayLimit, 'days').format('YYYY-MM-DD');
      const dayFile = getFilesFile(day);
      const fileIsExists = await fse.pathExists(dayFile);
      if (fileIsExists) {
        const filesSummary = await getFilesSummary(day);
        break;
      } else {
        dayLimit++;
      }
...
export async function saveFilesSummary(filesSummary: FilesSummary) {
  ...
  sendLog();
  await fse.writeJson(file, filesSummary, { spaces: 2 });
}
